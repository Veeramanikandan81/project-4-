        ...
        <?php
            ...
            ?>
               <td style="background-color : <?php echo ($data["mw"]<=0)?'red' : 'green'?>"><?php 
                ...
                <td style="background-color: <?php echo($data["ry_volt"]<=0)?'red':'green'?>";> <?php 
                    $res['yb_volt']= $data['yb_volt']/1000;
                    printf("%0.3f",$res['yb_volt']);?></td>
                <td style="background-color: <?php echo($data["br_volt"]<=0)?'red' :'green'?>";> <?php 
                    $res['br_volt']= $data['br_volt']/1000;
                    printf("%0.3f",$res['br_volt']);?></td>
                <td style="background-color: <?php echo($data["r_cur"]<=0)?'red':'green'?>"; ><?php 
                    $res['r_cur']= $data['r_cur']/10;
                    printf("%0.3f",$res['r_cur']);?></td>
                <td style="background-color: <?php echo($data["y_cur"]<=0)?'red':'green' ?>"; ><?php 
                    $res['y_cur']= $data['y_cur']/10;
                    printf("%0.3f",$res['y_cur']);?></td>
                <td style="background-color: <?php echo($data["b_cur"]<=0)?'red':'green'?>"; ><?php
                     $res['b_cur']= $data['b_cur']/10;
